...
export default function YetkilerSayfasi() {
    ...
    const fetchAll = useCallback(async () => {
        ...
        const promises =
            yetkiTipi === "BUTON" ? [...basePromises, ...butonPromises] : [...basePromises, ...kolonPromises];
    ...
    const getEffectiveButonIzin = (butonId) => {
        ...
        if (Object.prototype.hasOwnProperty.call(overrideMap, butonId)) return !!overrideMap[butonId];
    ...
    const getEffectiveKolonPerm = (kolonId) => {
        ...
        if (Object.prototype.hasOwnProperty.call(overrideMap, kolonId)) return overrideMap[kolonId] ?? fallback;
    ...
    const upsertRolButon = async (butonId, izin) => {
        const { error } = await supabase
            ...
            .upsert({ rol_id: rolId, buton_id: butonId, izin: !!izin }, { onConflict: "rol_id,buton_id" });
        if (error) throw error;
    ...
    const upsertRolKolon = async (kolonId, gorebilir, duzenleyebilir) => {
        ...
        setRolKolonMap((prev) => {
            const out = { ...(prev || {}) };
            out[rolId] = { ...(out[rolId] || {}), [kolonId]: { gorebilir: g, duzenleyebilir: d } };
            return out;
        });
    };
    ...
    const setKolonPerm = async (kolonId, next) => {
        ...
        try {
            if (mode === "ROL") await upsertRolKolon(kolonId, next.gorebilir, next.duzenleyebilir);
            else await upsertUserKolon(kolonId, next.gorebilir, next.duzenleyebilir);
        } catch (e) {
            console.error("Kaydetme hatası:", e?.message || e);
            await fetchAll();
        } finally {
    ...
    }, [butonlar, ekranById, query, onlyAllowed, mode, rolId, kullaniciId, rolButonMap, userButonMap, selectedUser]);
    ...
    const groupedButonlar = useMemo(() => {
        ...
        return grupKeys.map((gr) => {
            ...
            ekranList.forEach((x) => {
                x.butonlar.sort(
                    (a, b) => (a.sira ?? 0) - (b.sira ?? 0) || String(a.ad || "").localeCompare(String(b.ad || ""), "tr")
                );
            });
    ...
    const groupedKolonlar = useMemo(() => {
        ...
        filteredKolonlar.forEach((k) => {
            ...
            if (!out[grup][ek.id].gridler[gr.id]) out[grup][ek.id].gridler[gr.id] = { grid: gr, kolonlar: [] };
    ...
    const totalVisibleItems = yetkiTipi === "BUTON" ? filteredButonlar.length : filteredKolonlar.length;
    const totalAllowedItems = useMemo(() => {
        if (yetkiTipi === "BUTON") return filteredButonlar.filter((x) => getEffectiveButonIzin(x.id)).length;
        return filteredKolonlar.filter((x) => {
            const p = getEffectiveKolonPerm(x.id);
            return p.gorebilir || p.duzenleyebilir;
        }).length;
    }, [yetkiTipi, filteredButonlar, filteredKolonlar, mode, rolId, kullaniciId, rolButonMap, userButonMap, rolKolonMap, userKolonMap, selectedUser]);
    ...
    }, [yetkiTipi, filteredKolonlar, mode, rolId, kullaniciId, rolKolonMap, userKolonMap, selectedUser]);

    const subjectLabel = mode === "ROL"
        ? selectedRole
        ...
        return (
            <Box sx={pageSx}>
                <Stack alignItems="center" justifyContent="center" sx={{ minHeight: "60vh" }} spacing={2}>
                    <CircularProgress />
                    <Typography sx={{ color: "#cbd5e1", fontWeight: 800 }}>Yetkiler yükleniyor…</Typography>
                </Stack>
            </Box>
        );
    }
        ...
        return (
            <Box sx={pageSx}>
                <Paper elevation={0} sx={lockedPanelSx}>
                    ...
                    <Typography sx={{ color: "#fff", fontWeight: 900, fontSize: 22 }}>Erişim Yok</Typography>
                    <Typography sx={{ color: "rgba(255,255,255,0.68)", mt: 1, fontSize: 14, maxWidth: 560 }}>
                        Bu sayfaya erişim yetkiniz yok. Yönetici panelinden bu ekran için rol veya kullanıcı bazlı izin tanımlanmalıdır.
                    </Typography>
                </Paper>
            </Box>
        );
        ...
        <Box sx={pageSx}>
            <Stack spacing={2.2}>
                <Box sx={heroSx}>
                    ...
                    >
                        <Stack spacing={1.1} sx={{ maxWidth: 840 }}>
                            <Chip
                            ...
                            <Typography variant="h3" sx={heroTitleSx}>
                                Buton ve kolon yetkilerini
                                <Box component="span" sx={{ color: "#7dd3fc" }}> daha hızlı, daha net </Box>
                                yönetin.
                            </Typography>
                            <Typography sx={heroDescSx}>
                                Rol veya kullanıcı bazında görünürlük ve düzenleme yetkilerini modern bir kontrol paneli ile yönetin.
                                Arama yapın, filtreleyin ve değişiklikleri anında uygulayın.
                            </Typography>
                        </Stack>
                        ...
                        <Stack spacing={2}>
                            ...
                            >
                                <Stack spacing={1.2}>
                                    ...
                                    <Stack direction={{ xs: "column", md: "row" }} spacing={1.2} alignItems={{ md: "center" }}>
                                        <SegmentedMode
                                            ...
                                            options={[
                                                { value: "BUTON", label: "Buton", icon: <AppsRounded sx={{ fontSize: 18 }} /> },
                                                { value: "KOLON", label: "Kolon", icon: <GridViewRounded sx={{ fontSize: 18 }} /> },
                                            ]}
                                        />
                                        ...
                                        <SegmentedMode
                                            ...
                                            onChange={setMode}
                                            options={[
                                                { value: "ROL", label: "Rol", icon: <ShieldRounded sx={{ fontSize: 18 }} /> },
                                                { value: "KULLANICI", label: "Kullanıcı", icon: <PersonRounded sx={{ fontSize: 18 }} /> },
                                            ]}
                                        />
                                    </Stack>
                                </Stack>
                                ...
                                <Paper elevation={0} sx={contextBadgeSx}>
                                    <Typography sx={contextLabelSx}>{mode === "ROL" ? "Aktif rol" : "Aktif kullanıcı"}</Typography>
                            ...
                            <Stack direction={{ xs: "column", xl: "row" }} spacing={1.2} alignItems={{ xl: "center" }}>
                                {mode === "ROL" ? (
                                    <FormControl sx={{ minWidth: { xs: "100%", md: 260 } }}>
                                        ...
                                        <Select value={rolId} label="Rol" onChange={(e) => setRolId(e.target.value)} sx={selectSxModern}>
                                            {roller.map((r) => (
                                    ...
                                    <FormControl sx={{ minWidth: { xs: "100%", md: 360 } }}>
                                        ...
                                        <Select
                                            ...
                                            sx={selectSxModern}
                                            renderValue={(val) => {
                                                const u = kullanicilar.find((x) => String(x.id) === String(val));
                                                if (!u) return "Kullanıcı seçin";
                                                return u.mail ? `${u.kullanici} (${u.mail})` : u.kullanici;
                                            }}
                                        >
                                            {kullanicilar.map((u) => (
                                                <MenuItem key={u.id} value={String(u.id)}>
                                                    {u.mail ? `${u.kullanici} (${u.mail})` : u.kullanici}
                                                </MenuItem>
                                            ))}
                                        </Select>
                                    </FormControl>
                                ...
                                {yetkiTipi === "KOLON" ? (
                                    <FormControl sx={{ minWidth: { xs: "100%", md: 340 } }}>
                                        ...
                                        <Select value={gridId} label="Grid" onChange={(e) => setGridId(e.target.value)} sx={selectSxModern}>
                                ...
                                <TextField
                                    ...
                                    placeholder={yetkiTipi === "BUTON" ? "Buton, ekran veya kod ara…" : "Kolon, grid veya kod ara…"}
                                    size="small"
                                    sx={searchSxModern}
                                    InputProps={{
                                        startAdornment: (
                                ...
                                <ButtonBase onClick={() => setOnlyAllowed((p) => !p)} sx={filterPillSx(onlyAllowed)}>
                                    <Stack direction="row" spacing={1} alignItems="center">
                                        <TuneRounded sx={{ fontSize: 18 }} />
                                        <Typography sx={{ fontWeight: 900, fontSize: 13 }}>Sadece izinli</Typography>
                                        <Switch checked={onlyAllowed} tabIndex={-1} disableRipple />
                                    </Stack>
                                </ButtonBase>
                                ...
                                <Chip
                                    label={loading ? "Yükleniyor..." : `${totalVisibleItems} ${yetkiTipi === "BUTON" ? "kayıt" : "kolon"}`}
                                    sx={countChipSx}
                                />
                            </Stack>
                        </Stack>
                    ...
                    <Box sx={{ p: { xs: 1.2, md: 2 } }}>
                        {loading ? (
                            <Stack alignItems="center" sx={{ py: 10 }} spacing={1.5}>
                                <CircularProgress />
                                <Typography sx={{ color: "#94a3b8", fontWeight: 800 }}>Veriler yükleniyor…</Typography>
                            </Stack>
                        ) : yetkiTipi === "BUTON" ? (
                            groupedButonlar.length === 0 ? (
                                <Paper elevation={0} sx={emptyPaperSx}>
                                    <Typography sx={{ color: "#cbd5e1", fontWeight: 800 }}>Sonuç bulunamadı.</Typography>
                                </Paper>
                            ) : (
                                <Stack spacing={2}>
                                    {groupedButonlar.map((g) => (
                                        <Stack key={g.grup} spacing={1.1}>
                                            <Stack direction="row" spacing={1} alignItems="center">
                                                <Typography sx={sectionTitleSx}>{g.grup}</Typography>
                                                <Chip size="small" label={`${g.ekranlar.length} ekran`} sx={miniChipSx} />
                                            </Stack>
                                                ...
                                                <Paper key={ekran.id} elevation={0} sx={groupCardSx}>
                                                    <Stack spacing={1.3}>
                                                        ...
                                                        >
                                                            ...
                                                            <Chip size="small" label={`${blist.length} buton`} sx={miniChipSx} />
                                                        </Stack>
                                                        ...
                                                        <Box sx={rowsGridSx}>
                                                            {blist.map((b) => {
                                                                ...
                                                                return (
                                                                    <Paper key={b.id} elevation={0} sx={itemCardSx}>
                                                                        <Stack spacing={1.4}>
                                                                            <Box>
                                                                                <Typography sx={itemTitleSx}>{b.ad}</Typography>
                                                                                <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 0.6 }}>
                                                                                    <Chip size="small" label={b.kod || b.id} sx={codeChipSx} />
                                                                                    <Typography sx={mutedTinySx}>Tek tıkla izin yönetimi</Typography>
                                                                                </Stack>
                                                                            </Box>
                                                                            ...
                                                                            <PermissionSwitch
                                                                                ...
                                                                                onChange={(ev) => toggleButonIzin(b.id, ev.target.checked)}
                                                                                label="Buton erişimi"
                                ...
                                {groupedKolonlar.map((g) => (
                                    <Stack key={g.grup} spacing={1.1}>
                                        ...
                                        {g.ekranlar.map(({ ekran, gridList }) => (
                                            <Stack key={ekran.id} spacing={1.2}>
                                                        ...
                                                        <Stack spacing={1.3}>
                                                                ...
                                                                <Stack direction="row" spacing={1.1} alignItems="center">
                                                                    ...
                                                                    <Box>
                                                                        ...
                                                                        <Stack direction="row" spacing={0.6} alignItems="center" flexWrap="wrap">
                                                                            ...
                                                                            <KeyboardArrowRightRounded sx={{ fontSize: 15, color: "#64748b" }} />
                                                                            <Typography sx={groupSubSx}>{grid.kod}</Typography>
                                                                        </Stack>
                                                                    </Box>
                                                                </Stack>
                                                                <Chip size="small" label={`${klist.length} kolon`} sx={miniChipSx} />
                                                            </Stack>
                                                            ...
                                                            <Box sx={rowsGridSx}>
                                                                {klist.map((k) => {
                                                                    const p = getEffectiveKolonPerm(k.id);
                                                                    return (
                                                                        <Paper key={k.id} elevation={0} sx={itemCardSx}>
                                                                            <Stack spacing={1.4}>
                                                                                <Box>
                                                                                    <Typography sx={itemTitleSx}>{k.ad}</Typography>
                                                                                    <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 0.6 }}>
                                                                                        <Chip size="small" label={k.kod} sx={codeChipSx} />
                                                                                        <Typography sx={mutedTinySx}>Kolon görünürlük ve düzenleme</Typography>
                                                                                    </Stack>
                                                                                </Box>
                                                                                ...
                                                                                <Stack direction={{ xs: "column", sm: "row" }} spacing={1.1}>
                                                                                    <PermissionSwitch
                                                                                        ...
                                                                                        onChange={(ev) => {
                                                                                            ...
                                                                                            setKolonPerm(k.id, {
                                                                                                gorebilir: nextGor,
                                                                                                duzenleyebilir: nextGor ? p.duzenleyebilir : false,
                                                                                            });
                                                                                        }}
                                                                                        label="Görüntüleme"
                                                                                    ...
                                                                                    <PermissionSwitch
                                                                                        ...
                                                                                        onChange={(ev) => {
                                                                                            ...
                                                                                            setKolonPerm(k.id, {
                                                                                                gorebilir: nextEdit ? true : p.gorebilir,
                                                                                                duzenleyebilir: nextEdit,
                                                                                            });
                                                                                        }}
                                                                                        label="Düzenleme"
                                                                                        hint="Açıldığında görüntüleme izni otomatik aktif kalır."
                                                                                        accent="#a78bfa"
                                                                                    />
                                                                                </Stack>
                                                                            </Stack>
...
const filterPillSx = (active) => ({
    ...
    minHeight: 44,
    border: '1px solid',
    borderColor: active ? 'rgba(52,211,153,0.30)' : 'rgba(255,255,255,0.08)',
    color: active ? '#d1fae5' : '#cbd5e1',
    bgcolor: active ? 'rgba(52,211,153,0.10)' : 'rgba(255,255,255,0.03)',
